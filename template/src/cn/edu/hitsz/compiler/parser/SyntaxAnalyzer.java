...
public class SyntaxAnalyzer {
    ...
    private Stack<Symbol> symbol_stack = new Stack<>();
     ...
     *               需要注意的是, 在实现驱动程序的过程中, 你会需要面对只读取一个 token 而不能消耗它的情况, 在自行设计的时候请加以考虑此种情况
     */
    ...
    public void run() {
        //初始化两个栈
        symbol_stack.push(new Symbol(Token.simple(TokenKind.eof())));
        status_stack.push(lrTable.getInit());
        Iterator<Token> l = list.iterator();
        while (l.hasNext()){
            Token t = list.get(inputPtr);
            Status top =status_stack.peek();
            Action lAction = lrTable.getAction(top, t);
            switch (lAction.getKind()) {
                case Reduce -> {
                    Production p = lAction.getProduction();
                  //  callWhenInReduce(status_stack.peek(), p);
                    int leng = p.body().size();
                    for (int i = 0; i < leng; i++) {
                        symbol_stack.pop();
                        status_stack.pop();
                    }
                    symbol_stack.push(new Symbol(p.head()));
                    Status s = lrTable.getGoto(status_stack.peek(), p.head());
                    callWhenInReduce(s, p);
                    status_stack.push(s);
                }
                case Shift -> {
                    Status lStatus = lAction.getStatus();
                    symbol_stack.push(new Symbol(t));
                    status_stack.push(lStatus);
                    inputPtr++;
                    callWhenInShift(lStatus, t);
                }
                case Accept -> {
